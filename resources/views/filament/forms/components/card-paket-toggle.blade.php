<x-dynamic-component :component="$getFieldWrapperView()" :field="$field">
    ...
    @php
        $groupedPakets = \App\Models\Paket::whereBetween('tgl_paket', [$this->data['from_date'], $this->data['to_date']])
            ->orderBy('tgl_paket', 'asc')
            ->get()
            ->groupBy(function($date) {
                return \Carbon\Carbon::parse($date->tgl_paket)->format('F Y');
            });
    @endphp

    @foreach ($groupedPakets as $month => $pakets)
